...
public sealed class UiVirtualizedList : UiElement
{
    ...
    internal bool TryGetItemIndexAt(float pointerX, float pointerY, out int itemIndex)
    {
        itemIndex = -1;
        if (!ContainsPoint(pointerX, pointerY) || _items.Count == 0)
        {
            return false;
        }

        float localY = pointerY - LayoutBounds.Y + _scrollOffsetY;
